---
title: "."
subtitle: "- `r params$sujeto_obligado`"
author: ""
date: ""
output: 
  pagedreport::paged_windmill:
    front_img: "informe_portadas/portada_trim_x_organismo-1.png"
    logo: " "
    img_to_dark: FALSE
    logo_to_white: FALSE
...
df_transparencia_so <- read_csv(here(glue("bases/{ultima_base}"))) |> 
...
param_fecha <- unique(df_transparencia$periodo)[2]
...
En el presente informe se presentan los resultados alcanzados por `r params$sujeto_obligado`.
...
```{r}
## Resultado TOTAL
df_tabla_indice <- df_transparencia_so |> 
...
tabla_indiceTA_x_dimension <- df_transparencia_so |> 
  filter(item_tipo == "TA") |> 
...
tabla_indiceTP_x_dimension |> 
  ...
  fmt_percent(columns = prom_subitem, decimals = 1, sep_mark = ".", dec_mark = ",") |> 
  tab_style(
    style = list(
      cell_text(weight = "bold")),
    locations = cells_body(
      rows = equivalencia == "Índice de Transparencia Procativa"
    )
  ) |> 
  cols_align(align = "left", columns = equivalencia) |> 
  cols_align(align = "center", columns = prom_subitem) |> 
